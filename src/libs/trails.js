 ...
 * @Author: duanlinlin 
 * @Date: 2019-01-23 19:50:27 
 * @Last Modified by: duanlinlin
 * @Last Modified time: 2019-01-23 19:52:49
 */
...
var mouseX = (window.innerWidth - SCREEN_WIDTH - SIDE_WIDTH);
var mouseY = (window.innerHeight - SCREEN_HEIGHT - TOP_HEIGHT);
...
function documentMouseMoveHandler(event) {
    ...
    mouseX = event.clientX - (window.innerWidth - SCREEN_WIDTH - SIDE_WIDTH) * .5;
    mouseY = event.clientY - (window.innerHeight - SCREEN_HEIGHT - TOP_HEIGHT) * .5;
}
...
function canvasTouchStartHandler(event) {
    ...
    if (event.touches.length == 1) {
        ...
        mouseX = event.touches[0].pageX - (window.innerWidth - SCREEN_WIDTH - SIDE_WIDTH) * .5;
        mouseY = event.touches[0].pageY - (window.innerHeight - SCREEN_HEIGHT - TOP_HEIGHT) * .5;
    }
}
...
function windowResizeHandler() {
    ...
    canvas.style.left = (window.innerWidth - SCREEN_WIDTH - SIDE_WIDTH) * .5 + 'px';
    canvas.style.top = (window.innerHeight - SCREEN_HEIGHT - TOP_HEIGHT) * .5 + 'px';
}
...
export function destory() {
    ...
    canvas.removeEventListener('touchstart', canvasTouchStartHandler, false);
    canvas.removeEventListener('touchmove', canvasTouchMoveHandler, false);
